...
namespace WebApiCSharp.GenerateCodeFiles
{
    public class GenerateRos2Middleware
    {
        ...
        public GenerateRos2Middleware(PLPsData data, InitializeProject initProj)
        {
            ...
            try
            {
                GenerateFilesUtils.RunApplicationUntilEnd("ros2", rosWorkspaceSrcDirPath,
                    $"pkg create {"aos_ros2_middleware_auto"} --build-type ament_python --dependencies rclpy");
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
            ...
            Console.WriteLine(rosMiddlewareDirectory);
            ...
            GenerateFilesUtils.WriteTextFile(
                rosMiddlewareDirectory + "/aos_ros2_middleware_auto/" + ROS2_MIDDLEWARE_PACKAGE_NAME + "_node.py",
                Ros2MiddlewareFileTemplate.GetAosRos2MiddlewareNodeFile(data, initProj), true);
        }
    }
}
