        ...
        private static string GetHandleModuleFunctionV2(PLPsData data)
{
    ...
    foreach (RosGlue glue in data.RosGlues.Values)
    {
        ...
        if (glue.LocalVariablesInitializationFromGlobalVariables.Count > 0)
        {
            ...
            result += GenerateFilesUtils.GetIndentationStr(2, 4, "except Exception as e:");
            result += GenerateFilesUtils.GetIndentationStr(3, 4, "registerError(str(e), traceback.format_exc(), 'Action: " + glue.Name + ", illegalActionObs')");
            result += GenerateFilesUtils.GetIndentationStr(3, 4, "self.responseNotByLocalVariables = \"illegalActionObs\"");
...
" + GetHandleModuleFunctionV2(data) + pythonContainer.getFromNodeFile() + GetModuleResponseFunctionPartV2(data) +
                          pythonContainer.getRegister() + GetListenToMongoCommandsFunctionPartV2(data) + pythonContainer.timer_function() + check_LocalVars(data) + pythonContainer.Register_ModuleResponse()+ reset_navigation_vars(data)+
