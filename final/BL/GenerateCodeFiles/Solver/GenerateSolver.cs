...
namespace WebApiCSharp.GenerateCodeFiles
{
    public class GenerateSolver
    {
        ...
        public GenerateSolver(PLPsData data, InitializeProject initProj, Solver solverData)
        {
            ...
            GenerateFilesUtils.WriteTextFile(conf.SolverPath + "/src/solver/pomcp.cpp",
                SolverFileTemplate.GetPOMCP_File(conf.SolverGraphPDF_DirectoryPath,
                    initProj.SolverConfiguration.PolicyGraphDepth, initProj, data));
            ...
            if (initProj.SolverConfiguration.UseML)
            {
                ...
                GenerateFilesUtils.RunBashCommand(
                    "c++ -O3 -Wall -shared -std=c++17 -fPIC $(python3 -m pybind11 --includes) aos_domain_for_python.cpp -o aos_domain$(python3-config --extension-suffix)",
